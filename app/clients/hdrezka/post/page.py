...
class Page:
    ...
    async def get_page(self, page: PageNumber) -> list[InlineItem]:
        ...
        """
        items = BeautifulSoup(
            (await get_response("GET", self._page_format(page))).text, builder=_BUILDER
        ).find_all(class_="b-content__inline_item")
        if not items:
            raise EmptyPage(page)
        return [
            InlineItem(
                (a := (link := i.find(class_="b-content__inline_item-link")).find("a"))[
                    "href"
                ],
                a.text,
                link.find("div").text,
                i.find(class_="b-content__inline_item-cover")
                .find("img")
                .get("src", ""),
            )
            for i in items
