...
class Post:
        ...
        self.translator_id = (
            ...
            if (
                i := _response.split(
                    "sof.tv."
                    + {
                        "tv_series": "initCDNSeriesEvents",
                        "movie": "initCDNMoviesEvents",
                    }[self.type],
                    1,
                )[-1]
                .split(",")[1]
    ...
    def _get_type(self) -> str:
        return self._soup_inst.find("meta", property="og:type")["content"].removeprefix(
            "video."
        )
    ...
    def _get_translators(self) -> Translators:
        ...
        arr = (
            {
                child.text.strip(): int(child["data-translator_id"])
                for child in translators_list.find_all(recursive=False)
                if child.text
            }
            if translators_list
            else {}
    ...
    def _parts_urls(self) -> tuple[str]:
        self.other_parts_urls = (
            *(
                i.attrs["data-url"]
                for i in self._soup_inst.select(".b-post__partcontent_item[data-url]")
            ),
        )
