...
class PostInfo:
    ...
    def __init__(self, soup: BeautifulSoup):
        ...
        fields = {
            h2.text.strip().removesuffix(":"): raw_fields[i + 1]
            for i in range(0, len(raw_fields), 2)
            if (h2 := raw_fields[i].find("h2")) is not None
        }
    ...
    def _get_field_places(tag: Tag) -> tuple[Place, ...]:
        ...
        return (
            *(
                Place(a.text, _get_int(a.next_element.next_element))
    ...
    def _get_field_release(tag: Tag) -> Release:
        """Дата выхода"""
        return Release(
            day="" if tag.next_element is None else tag.next_element.text.strip(),
    ...
    def _form_view(self, table: list[Tag]) -> str:
        return "{0}\n\n{1}\n\n{2}".format(
            (
                self.title
                if self.orig_title == self.title
                else f"{self.title} ({self.orig_title})"
            ),
