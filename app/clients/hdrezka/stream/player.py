...
class PlayerBase:
    ...
    def _translator(self, translator_id: Optional[SupportsInt] = None) -> int:
        ...
        return (
            self.post.translators.ids[abs(translator_id)]
            if translator_id <= 0
            else translator_id
        )
...
class PlayerMovie(PlayerBase):
    ...
    async def get_stream(self, translator_id: Optional[SupportsInt] = None) -> URLs:
        ...
        return urls_from_ajax_response(
            await AJAX.get_movie(self.post.id, self._translator(translator_id))
        )
...
class PlayerSeries(PlayerBase):
    ...
    async def get_episodes(
        self, translator_id: Optional[SupportsInt] = None
    ) -> defaultdict[int, tuple[int, ...]]:
        """Returns available episodes"""
        episodes = BeautifulSoup(
            (await AJAX.get_episodes(self.post.id, self._translator(translator_id)))[
                "episodes"
            ],
            features=["html", "fast"],
        )
        result: defaultdict[int, tuple[int, ...]] = defaultdict(tuple)
        for i in episodes.find_all(
            class_="b-simple_episode__item", attrs=("data-season_id", "data-episode_id")
        ):
            result[int(i.get("data-season_id"))] += (int(i.get("data-episode_id")),)
        return result

    async def get_stream(
        self, season: int, episode: int, translator_id: Optional[SupportsInt] = None
    ) -> URLs:
        """Returns episode stream `URLs`"""
        return urls_from_ajax_response(
            await AJAX.get_stream(
                self.post.id, self._translator(translator_id), season, episode
            )
        )
